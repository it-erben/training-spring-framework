...
`@RestController` liefert Bücher als JSON aus, eigene Request- und
Response-DTOs (Records) trennen die Schnittstelle vom Domänenmodell,
Bean Validation prüft Eingaben, und ein `@RestControllerAdvice`
übersetzt fachliche Ausnahmen in die richtigen HTTP-Statuscodes.
...
Das Response-DTO enthält zusätzlich zum Nettopreis den berechneten
Bruttopreis (19 % Mehrwertsteuer, kaufmännisch gerundet):
...
| Modul | Inhalt |
| --- | --- |
| `sb-basics-web-demo-start` | Fachlogik fertig (`Book`, `BookService`, `BookNotFoundException`), aber ohne Web-Schicht — Controller, DTOs und Exception-Handler entstehen live. `TODO: Modul 02`-Marken im Code (IDE: TODO-Ansicht, Anker für die neuen Typen in `package-info.java`) verweisen mit Schrittnummern auf den Ablauf unten. |
| `sb-basics-web-demo-finished` | Vollständige REST-Schnittstelle inklusive Tests. |
...
zeigen. Vorab einmal ohne Änderung starten: Der Tomcat läuft auf Port
...
1. **Erster Controller:** `BookController` anlegen — `@RestController`,
   ...
   einen Konverter konfiguriert. Auffällig: Die Antwort zeigt das nackte
   Domänenmodell mit `netPrice`.

2. **Response-DTO:** `BookResponse`-Record mit zusätzlichem `grossPrice`
   ...
   den Bruttopreis — und die Schnittstelle ist vom Domänenmodell
   entkoppelt: `Book` kann sich ändern, ohne dass Clients es merken.

3. **Pfadvariable:** Einzelabruf ergänzen:
   ...
   oben an, und eine unbehandelte `RuntimeException` ist für Spring ein
...
4. **Exception-Handler:** `RestExceptionHandler` anlegen —
   ...
   zurückgibt. Derselbe `curl` liefert jetzt `404` mit
   `{"error":"Kein Buch mit ISBN 999 gefunden"}` — zentrale
   Fehlerbehandlung für alle Controller, kein `try/catch` im Controller.
...
5. **POST mit Validierung:** `BookRequest`-Record mit
   ...
   Ein gültiger POST liefert `201` mit `Location`-Header auf die neue
   Ressource:
   ...
   falsch war, bekommt der `RestExceptionHandler` noch einen Handler für
...
die Web-Schicht, die sie prüfen würden):
...
| Behauptung | Test |
| --- | --- |
| `GET /api/books` liefert `200`, jedes Buch enthält `grossPrice` | `listReturnsBooksWithGrossPrice` |
| `GET` auf bekannte ISBN liefert `200`; `grossPrice` = Nettopreis + 19 %, kaufmännisch gerundet | `knownIsbnReturnsBookWithGrossPrice` |
| `GET` auf unbekannte ISBN liefert `404` | `unknownIsbnReturnsNotFound` |
| `POST` mit gültigem Body liefert `201` mit `Location`-Header | `createReturnsCreated` |
